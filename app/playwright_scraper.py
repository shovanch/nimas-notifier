...
def get_availability_via_playwright(
    ...
    """Scrape availability using headless browser by finding serial_no row."""
        ...
        try:
            ...
            for r in range(rows.count()):
                ...
                if first == target:
                    ...
                    if not m:
                        raise ValueError(
                            f'Matched row but no integer found in availability cell: {raw!r}'
                        )
                    return int(m.group(0))
            raise ValueError(
                f'Row with first cell == {serial_no!r} not found in DOM.'
            )
        finally:
            browser.close()
